...
Format Fit:
Given the genre and angle stated in the CONCEPT BRIEF (or the user's brief if no CONCEPT BRIEF exists), which of the trending formats above is the strongest fit? Name the format, then one sentence explaining why it suits this genre and moment. If the CONCEPT BRIEF declares comedy, assess Fake-Explainer fit specifically.
...
Write to: `.context/runs/{task_id}/agent-reel-trend-scout.md`
...
ended: {ISO timestamp}
tags: [research, trends, {platform}, {niche}]
---

## Intent
{What niche/platform trend research was requested}
...
## Outcome
{Research file path; top trending audio/format/theme summary}

## Errors / Surprises
{Stale data, unreliable sources, search failures — or "ไม่มี"}

## Root cause (only if status=failed)
{Actual cause — NOT a raw error message}

## For next agent
{Imperative: which trend findings are most relevant to the concept brief; any "fading" trends to avoid — must NOT be empty}
```
